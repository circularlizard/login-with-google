...
class Authenticator {
	...
	public function authenticate( stdClass $user ): WP_User {
		if ( ! property_exists( $user, 'email' ) ) {
			throw new InvalidArgumentException( esc_html__( 'Email needs to be present for the user.', 'oauth-login' ) );
		}
		...
		if ( email_exists( $user->email ) ) {
			 ...
			 * @param stdClass $user User data object returned by Google.
			 */
			do_action( 'oauth.login_user_logged_in', $user_wp, $user );
		 ...
		 * @since 1.0.0
		 */
		return apply_filters( 'oauth.login_register_user', $this->maybe_create_username( $user ) );
	}
	...
	public function register( stdClass $user ): ?WP_User {
		...
		if ( ! $register ) {
			throw new Exception( esc_html__( 'Registration is not allowed.', 'oauth-login' ) );
		}
		...
		try {
			...
			if ( empty( $whitelisted_domains ) || $this->can_register_with_email( $user->email ) ) {
				 ...
				 */
				do_action( 'oauth.login_user_created', $uid, $user );

				return get_user_by( 'id', $uid );
			}

			/* translators: %s is replaced with email ID of user trying to register */
			throw new Exception( sprintf( __( 'Cannot register with this email: %s', 'oauth-login' ), $user->email ) );
